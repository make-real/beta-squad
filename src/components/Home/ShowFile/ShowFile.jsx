...
const ShowFile = ({ selectedSpaceId, ShowFile }) => {
  ...
  const handleAddLinks = async () => {
    ...
    if (selectedSpaceId) {
      try {
        const res = await add_file_link(selectedSpaceId, linkData);
        console.log(res);
        ...
        toast.error(`${err?.link},Please provide http/https link`, {
          autoClose: 3000,
        });
      }
    }
  ...
  return (
    <div
      style={{}}
      className={`overflow-y-scroll h-full max-h-screen  no-scrollbar custom-shadow bg-[#ECECEC80] py-10 px-4 rounded-2xl`}
    >
