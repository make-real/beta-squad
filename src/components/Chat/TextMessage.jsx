  ...
  return (
    ...
    >
      ...
      >
        ...
        {msg?.sendingFailed && <h1 className="text-red-500">ending Failed!</h1>}
        ...
        {msg.sender._id === userId && (
          <div className="px-1.5 hover:text-teal-400 tooltip-box group cursor-pointer">
            ...
            <div className="group-hover:scale-100 scale-0 origin-top-right transition-transform absolute right-[30%] top-[20px] bg-white normal-shadow rounded-[10px] flex flex-col z-20 text-sm">
              ...
              <div onClick={handleDelete} className="flex items-center gap-[15px] px-5 w-32 py-[17px] hover:bg-[#FB397F10] cursor-pointer">
  ...
  try {
    message?.content?.attachments?.map((src, idx) => {
      ...
      if (["png", "jpeg", "jpg", "ttif", "gif", "webp", "svg"].includes(extension)) {
        ...
        return (
          <div className="mb-2">
            <a className="underline text-blue-600 hover:text-blue-800 visited:text-purple-600" target="_blank" rel="noreferrer" href={src}>
              {src}
            </a>
          </div>
        );
...
const TextMessage = ({ messageToRespond, setMessageToRespond, forComment, comments }) => {
