...
export const addCard = createAsyncThunk("addCard/allboard",
...
export const DeleteCard = createAsyncThunk("deleteCard/allboard",
async({spaceId, listId, cardId})=>{
  try {
    const res = await useAxios.delete(`/spaces/${spaceId}/board/${listId}/card/${cardId}`)
    if(res.data){
      return res.data
    }
...
export const UpdatedCard = createAsyncThunk("UpdatedCard/allboard",
async({spaceId,listId,cardId,cardObj})=>{
  try {
    const res = await useAxios.patch(`/spaces/${spaceId}/board/${listId}/card/${cardId}`, cardObj)
    if(res.data){
      return res.data
    }
